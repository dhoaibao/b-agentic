...
- When a request or skill route requires b-plan, b-research, or b-debug, do not execute that Architect-owned work; route to the Architect instead: after a fresh \`list-cwd\` with the absolute project \`cwd\` shows exactly one other peer, issue exactly one blocking \`ask\` with that \`cwd\` and omit \`to\`, then stop and wait for the result. Never use \`send\` plus \`ask\` for the same exchange. A zero/multiple peer roster or unavailable intercom is a coordination gap; remain stopped until an approved-plan handoff, research result, or diagnosis handoff arrives.
- When the explicit executor role is active, every completed Executor-owned task that leaves a tracked or relevant untracked/derived worktree candidate requires independent b-review immediately and before any final task response. After required checks pass, freeze that candidate. Use a fresh \`list-cwd\` with the absolute project \`cwd\`; it must show exactly one other peer. Issue exactly one blocking \`ask\` with that \`cwd\`, omit \`to\`, and send the compact frozen-candidate handoff for independent b-review; then stop edits and do not report completion pending review. Never pair a review \`send\` with that \`ask\`. No-change tasks, including PR prose, do not require changed-code review. After receiving a valid ready b-review disposition, emit B_AGENTIC_TASK_COMPLETE on its own line immediately before the final task response so only the executor receives the task-completion notification. Report a coordination gap rather than a final result for zero/multiple peers or an unavailable intercom; never provision, reset, or guess a peer.
