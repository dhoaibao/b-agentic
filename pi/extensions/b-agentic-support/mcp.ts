...
export type McpToolApprovalDecision =
  "allow_once" | "allow_for_session" | "deny" | "abstain";
export type McpToolApprovalOrigin =
  "proxy" | "direct" | "script" | "resource" | "iframe";
export type McpToolApprovalRequest = {
  ...
  signal?: AbortSignal;
  claim: (
    handler: () => McpToolApprovalDecision | Promise<McpToolApprovalDecision>,
  ) => boolean;
};

export const MCP_TOOL_APPROVAL_REQUEST_EVENT =
  "pi-mcp-adapter:tool-approval-request";

const MCP_CONDITIONAL_ARGUMENT_KEY_SETS: Record<
  string,
  ReadonlySet<string>
> = Object.fromEntries(
  Object.entries(MCP_CONDITIONAL_ARGUMENTS).map(([tool, keys]) => [
    tool,
    new Set(keys),
  ]),
);
...
const PLAYWRIGHT_VERIFY_ELEMENT_VISIBLE_KEYS = new Set([
  "accessibleName",
  "role",
]);
const PLAYWRIGHT_VERIFY_LIST_VISIBLE_KEYS = new Set([
  "element",
  "items",
  "target",
]);
const PLAYWRIGHT_VERIFY_TEXT_VISIBLE_KEYS = new Set(["text"]);
const PLAYWRIGHT_VERIFY_VALUE_KEYS = new Set([
  "element",
  "target",
  "type",
  "value",
...
function managedDirectMcpTool(
  toolName: string,
): ManagedDirectMcpTool | undefined {
  if (!toolName.startsWith("mcp__")) return undefined;
  for (const server of [...MANAGED_MCP_SERVERS].sort(
    (left, right) => right.length - left.length,
  )) {
    const normalized = server.replace(/-/g, "_");
    for (const separator of ["__", "_"]) {
      // retain prior generated names for compatibility
...
export function isProjectConfinedPath(
  pathValue: unknown,
  requireFile = false,
): boolean {
  if (
    ...
    !pathValue ||
    isProtectedLocalPath(pathValue)
  )
    return false;
  try {
...
export function isSafeFirecrawlScrapeOptions(
  input: Record<string, unknown>,
): boolean {
  return (
    hasOnlyKeys(input, FIRECRAWL_SCRAPE_OPTION_KEYS) &&
...
export function isSafeFirecrawlSearch(input: Record<string, unknown>): boolean {
  return (
    hasOnlyKeys(
      input,
      MCP_CONDITIONAL_ARGUMENT_KEY_SETS["firecrawl:firecrawl_search"],
    ) &&
    ...
    (input.scrapeOptions === undefined ||
      (isPlainObject(input.scrapeOptions) &&
        isSafeFirecrawlScrapeOptions(input.scrapeOptions)))
  );
}
...
export function isSafeFirecrawlMap(input: Record<string, unknown>): boolean {
  return (
    hasOnlyKeys(
      input,
      MCP_CONDITIONAL_ARGUMENT_KEY_SETS["firecrawl:firecrawl_map"],
    ) &&
...
export function isSafeFirecrawlExtract(
  input: Record<string, unknown>,
): boolean {
  return (
    hasOnlyKeys(
      input,
      MCP_CONDITIONAL_ARGUMENT_KEY_SETS["firecrawl:firecrawl_extract"],
    ) &&
    Array.isArray(input.urls) &&
    input.urls.length > 0 &&
    input.urls.length <= 10 &&
    input.urls.every(isSafeWebUrl) &&
    input.allowExternalLinks !== true &&
    input.enableWebSearch !== true &&
    input.includeSubdomains !== true
  );
}

export function isConditionallyTrustedTool(
  server: string,
  base: string,
  input: unknown,
): boolean {
...
export function isTrustedManagedTool(
  server: string,
  toolName: string,
  input?: unknown,
): boolean {
...
export function gatewayToolMatchesServer(
  server: string,
  toolName: string,
): boolean {
...
export function isMcpProxyToolExecution(
  input: unknown,
): input is McpProxyToolExecution {
  if (!isPlainObject(input)) return false;
  if (!Object.keys(input).every((key) => MCP_PROXY_EXECUTION_FIELDS.has(key)))
    return false;
...
function isDirectClassifiedManagedTool(
  toolName: string,
  input: unknown,
  server: "codegraph",
): boolean {
...
export function isMcpOrCustomTool(toolName: string, input?: unknown): boolean {
  ...
  if (toolName === "preview_markdown" && isTrustedPreviewMarkdownCall(input))
    return false;
...
export function approvalPreview(value: unknown): string {
  ...
  // eslint-disable-next-line no-control-regex -- intentional control-character sanitization
...
export function isMcpToolApprovalRequest(
  value: unknown,
): value is McpToolApprovalRequest {
