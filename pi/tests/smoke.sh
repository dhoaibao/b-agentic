...
const usePane = (pane) => { process.env.TMUX_PANE = pane; return roleTest.terminalPaneId(); };
const readPaneRole = (pane) => JSON.parse(readFileSync(roleTest.paneRolePath(root, pane), 'utf8')).role;
expect(roleTest.terminalPaneId({ TMUX_PANE: '%3', TMUX: '/tmp/tmux-1000/default,42,0' }, 4242).includes('%3'), 'a tmux pane must identify the terminal');
expect(roleTest.terminalPaneId({ WEZTERM_PANE: '7' }, 4242).includes('7'), 'a supported terminal pane variable must identify the terminal');
const fallbackPaneId = roleTest.terminalPaneId({}, 4242);
expect(fallbackPaneId === 'ppid:4242' || fallbackPaneId.startsWith('tty:'), 'a terminal without pane variables must fall back to its tty or parent process');
expect(roleTest.paneRolePath(root, 'pane-a') !== roleTest.paneRolePath(root, 'pane-b'), 'each pane must own an independent durable record');
expect(roleTest.paneRolePath(root, 'pane-a') !== roleTest.paneRolePath(path.join(root, 'other'), 'pane-a'), 'each project must own an independent durable record');
...
const contendingWriter = (pane, role) => new Promise((resolveWriter, rejectWriter) => {
  const child = spawn(process.execPath, [...process.execArgv, '--input-type=module', '-e', `
    const { savePaneRole } = await import(${JSON.stringify(pathToFileURL(path.join(installedRoot, 'b-agentic-support', 'role-store.ts')).href)});
    for (let index = 0; index < 40; index += 1) savePaneRole(${JSON.stringify(root)}, ${JSON.stringify(role)}, ${JSON.stringify(pane)}, ${JSON.stringify(contentionDir)});
  `], { stdio: 'inherit' });
