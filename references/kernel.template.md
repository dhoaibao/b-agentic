...
- `b-commit` does not initiate changed-code review for the prepared candidate or commit plan, regardless of group count or pre-existing staged paths; it self-audits the snapshot and plan instead. An explicit request to review before committing routes to `b-review` separately.
