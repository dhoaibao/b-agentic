...
- Do not store auth/session state or other sensitive run artifacts under repo-local `.b-agentic/` unless the user explicitly opts into repo-local persistence. Use the active runtime's user-scope b-agentic directory or temp scratch path instead by default (see §8 for runtime-specific paths).
