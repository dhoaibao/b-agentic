...
Use this wording pattern in installed runtime skills when a gate is required: `Read ../../b-agentic/references/contract/<file>.md before <action>`. For a per-skill `reference.md`, use: `Read ./reference.md before <action>`. Keep schemas in this contract; the skill owns only the local trigger for reading them.
