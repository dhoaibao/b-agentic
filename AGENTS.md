...
- Shared contract behavior: edit `references/contract/runtime.md`, `references/contract/safety-tools.md`, or `references/contract/shell-tools.md`.
...
- Keep `references/contract/` limited to `runtime.md`, `safety-tools.md`, `shell-tools.md`, `kernel.template.md`, and `mcp_operations.yaml` unless a new file clearly removes more complexity than it adds.
...
For install or runtime-home evidence after adapter changes, also use `scripts/smoke-install.sh`, `scripts/skill-doctor.sh --runtime=<name>`, `scripts/mcp-doctor.sh --runtime=<name>`, and `scripts/runtime-acceptance.sh --runtime=<name> --production` when release readiness or live-session behavior is in scope.
