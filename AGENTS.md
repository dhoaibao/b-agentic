# b-agentic — Agent Workflow Kernel Authoring
...
- `install.sh` deploys runtime files to `~/.config/opencode/`, always writes `b-agentic/AGENTS.md`, and replaces `AGENTS.md` only when missing or approved.
...
- `global/AGENTS.md` — Runtime kernel source (installs as `b-agentic/AGENTS.md`)
...
- If a skill points at a shared reference, ensure `install.sh` syncs it into `~/.config/opencode/references/b-agentic/`.
...
Fallbacks: if required tools are unavailable, read `references/b-agentic/runtime-contract.md` §4 before applying fallbacks. Skills add only skill-specific stop/degrade behavior.
