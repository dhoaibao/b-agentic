...
def kimi_server_status(server: str, config: dict) -> str:
    servers = config.get("mcpServers", {})
    entry = servers.get(server)
    if not isinstance(entry, dict):
        return "missing: config entry not installed"
    ...
    if server == "context7":
        ...
        if isinstance(headers, dict) and isinstance(headers.get("CONTEXT7_API_KEY"), str) and headers.get("CONTEXT7_API_KEY"):
            return "ready: CONTEXT7_API_KEY configured in Kimi MCP config"
        if entry.get("bearerTokenEnvVar") == "CONTEXT7_API_KEY":
            return "ready: CONTEXT7_API_KEY env binding configured in Kimi MCP config" if env_var_present("CONTEXT7_API_KEY") else "blocked: missing CONTEXT7_API_KEY; env binding configured in Kimi MCP config"
        return "blocked: missing CONTEXT7_API_KEY"
    if server == "codegraph":
        if entry.get("command") != "codegraph" or not list_matches(entry.get("args"), ["serve", "--mcp"]):
            return "blocked: invalid codegraph launcher"
        return "ready: codegraph command found" if command_ready("codegraph") else "blocked: install codegraph"
    ...
    env_section = entry.get("env", {})
    if not (isinstance(env_section, dict) and isinstance(env_section.get(env_key), str) and env_section.get(env_key)):
        issues.append(f"set {env_key} in Kimi MCP config")
    if issues:
        return f"blocked: {join_issues(issues)}"
    return ready_status(f"pnpm and {env_key} configured in Kimi MCP config", pinned_status)
...
def resolve_config_path(runtime: dict, home: Path) -> Path:
    ...
    if schema_family == "kimi-mcp-json":
        kimi_home = os.environ.get("KIMI_CODE_HOME")
        if kimi_home:
            return Path(kimi_home).expanduser() / "mcp.json"
        return home / ".kimi-code" / "mcp.json"
...
def main() -> int:
    ...
    elif schema_family == "kimi-mcp-json":
        config = load_json(config_path)
        status_fn = kimi_server_status
    else:
        config = load_json(config_path)
