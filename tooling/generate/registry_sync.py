...
def validate_skills(skills: list[dict[str, Any]]) -> list[str]:
    errors: list[str] = []
...
def render_outputs(skills: list[dict[str, Any]], policy: dict[str, Any]) -> dict[Path, str]:
...
def validate_regressions(
    skills: list[dict[str, Any]], capabilities: dict[str, Any], policy: dict[str, Any]
) -> list[str]:
    errors: list[str] = []
    invalid_skill = json.loads(json.dumps(skills))
    invalid_skill[0]["execution"] = {"mode": "invalid"}
    if not validate_skills(invalid_skill):
...
def sync_outputs(check: bool) -> int:
    ...
    errors = [*validate_skills(skills), *validate_policy(policy), *validate_capabilities(capabilities, policy)]
    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 1
    outputs = render_outputs(skills, policy)
...
def main() -> int:
    ...
    if args.self_test:
        errors = validate_regressions(load_skills(), load_capabilities(), load_policy())
        if errors:
            print("\n".join(errors), file=sys.stderr)
            return 1
