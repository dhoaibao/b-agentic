...
- Add `scripts/build-release.sh` to stage the release payload allowlist and emit `dist/b-agentic.tar.gz` with its SHA-256 checksum, `scripts/extract-release-notes.sh` to extract one exact Keep a Changelog section, a root `VERSION` file asserted equal to the newest CHANGELOG release heading, and a `release.yml` workflow that verifies the tag matches `VERSION`, reruns validation, smoke, and audit gates, and publishes both assets with extracted release notes.
...
- Narrow `--ref` from any tag-or-commit pin to `vYYYY.MM.DD` release tags only; commit-SHA and SemVer pins are rejected, and `releases/latest/download` URLs require a published release.
