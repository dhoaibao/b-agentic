...
- Pass MCP API keys in the syntax each host actually reads. The Codex block set a remote server's key under an `env` table Codex rejects for HTTP servers and wrote `${VAR}` strings Codex never expands; it now forwards the Context7 header with `env_http_headers` and stdio keys with `env_vars`. OpenCode received `${VAR}`, which it sends literally, and now gets `{env:VAR}`, with existing configs rewritten when they still hold the old managed placeholder.
- Restore the instruction file that `--replace-memory` displaced when b-agentic is uninstalled. A reinstall previously replaced the recorded backup with a copy of b-agentic's own kernel, and uninstall then deleted both the kernel and the backups directory. The original is now kept across reinstalls and copied back, or left in the backups directory with a warning when the managed kernel was edited — through the regular and the manifest-only uninstall alike.
- Keep uninstall able to remove what earlier installs merged. A reinstall that changed a merged configuration recorded a fresh backup already holding managed entries as the user's original, so uninstall treated those entries as user-owned and left them behind. The first install's record now wins.
- Stop reporting a failed `--sync` or uninstall as complete. Sync now stops at the first failed step, and uninstall fails the run and keeps its install metadata so a rerun can finish, instead of deleting the manifest after a skill removal failed. `--sync` also records newly installed skills in the manifest so uninstall and stale-skill pruning can find them.
...
- Ship a checksum-verified release-tarball installer: the piped script bootstraps only — it downloads the release tarball, verifies a 64-character SHA-256 digest, validates every archive entry against the published payload contract (rejecting absolute paths, `..` traversal, dotfiles, symlinks, and non-file members), and re-executes all state changes from inside the verified bundle with `--source-dir`.
- Install the release payload transactionally into `~/.b-agentic` with staging, backup, and rollback on failure or interruption, a line-2 `# B_AGENTIC_INSTALLER` marker guard that refuses unmanaged installers, and migration of legacy `git clone` checkouts wholesale to a reported `~/.b-agentic.backup.<timestamp>` directory without merging local modifications.
- Add `scripts/build-release.sh` to stage the release payload allowlist and emit `dist/b-agentic.tar.gz` with its SHA-256 checksum, `scripts/extract-release-notes.sh` to extract one exact Keep a Changelog section, a root `VERSION` file asserted equal to the newest CHANGELOG release heading, and a CI workflow that builds and publishes both assets with extracted release notes.
- Add `B_AGENTIC_RELEASE_URL` and `B_AGENTIC_CHECKSUM_URL` overrides for hermetic and mirrored installs, plus smoke coverage for checksum mismatches, unsafe archive entries, git-checkout migration, malformed `--ref` values, and release-URL construction, with the installer suite rebuilt around a built release fixture served over `file://`.
- Publish a release automatically after a passing push to `main`: the CI workflow derives an ordinal tag `vYYYY.MM.DD.N` (one plus the count of existing tags for that date, so same-day pushes never collide), runs the repository's quality, validation, smoke, and audit gates in the matrix job, then the release job checks shell syntax, stamps the runner-side `VERSION` with the full release version, extracts notes from that date's existing changelog section, and publishes both assets with the built-in `GITHUB_TOKEN` — no manual tagging, no personal access token, and no commit back to the branch.

### Changed

- Replace the installer's `git clone` transport with the checksum-verified release tarball: `git` is no longer an installer prerequisite, `B_AGENTIC_REPO` is retired, `--sync` re-downloads the latest verified release instead of pulling, `--dry-run` rehearses against a verified temporary payload, and `--update`/`--uninstall` reuse the installed source without any download.
