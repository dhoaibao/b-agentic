...
- Add `scripts/build-release.sh` to stage the release payload allowlist and emit `dist/b-agentic.tar.gz` with its SHA-256 checksum, `scripts/extract-release-notes.sh` to extract one exact Keep a Changelog section, a root `VERSION` file asserted equal to the newest CHANGELOG release heading, and a `release.yml` workflow that reruns validation, smoke, and audit gates, and publishes both assets with extracted release notes.
...
- Publish a release automatically on every push to `main`: the release workflow derives an ordinal tag `vYYYY.MM.DD.N` (one plus the count of existing tags for that date, so same-day pushes never collide), runs syntax, validation, smoke, and audit gates on the pristine tree before stamping the runner-side `VERSION` with the full release version, extracts notes from that date's existing changelog section, and publishes both assets with the built-in `GITHUB_TOKEN` — no manual tagging, no personal access token, and no commit back to the branch.
