...
If production impact, data loss, or security risk is active, read `{{runtime_reference_root}}/cards/before-edit.md` before identifying containment or asking for shared-environment action, then fall back to `{{runtime_reference_root}}/contract/06-safety.md` when needed. Treat containment as a reversible mitigation, not the final fix; record what remains unproven until root cause is confirmed.
...
- Read `{{runtime_reference_root}}/contract/07-execution.md` before applying the verification ladder, iteration cap, or skipped-check labels.
