...
3. Confirm root cause before fixing. Use probes only when cheaper evidence is insufficient and remove them.
4. Apply the smallest fix that addresses the confirmed cause.
5. Run the narrowest check proving the symptom changed. For perf, measure before and after.
