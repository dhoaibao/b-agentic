...
If no plan exists and the request fails the small-direct threshold, hand off to **b-plan**. Read `../../b-agentic/references/contract/06-safety.md` before editing.
...
Run the plan's check when available; otherwise use touched-file diagnostics and the narrowest relevant command. Classify failures before another edit. Read `../../b-agentic/references/contract/09-output.md` before emitting a status block.
