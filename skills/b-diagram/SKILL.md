...
- Use the bundled `~/.pi/agent/skills/b-diagram/diagram.py` with Python 3. Its `validate` command never writes; its `deliver` command atomically replaces only the named HTML target after validation succeeds.
...
3. Create a version-1 JSON source conforming to `~/.pi/agent/skills/b-diagram/schema.json`: stable node and edge IDs, labels, explicit edge endpoints, and optional evidence IDs. Cite evidence only as supplied metadata; never claim it was verified unless it was actually read.
4. Run `python3 ~/.pi/agent/skills/b-diagram/diagram.py validate <source.json>`. Repair only the reported local source defects.
5. Run `python3 ~/.pi/agent/skills/b-diagram/diagram.py deliver <source.json> <target.html>` only for the user-approved target. This writes one portable HTML/SVG artifact with no external assets.
