...
Read `{{runtime_reference_root}}/cards/routing.md`, `{{runtime_reference_root}}/cards/output-handoff.md`, and `{{runtime_reference_root}}/contract/11-session.md` before routing across phase skills. Keep exactly one phase owner active at a time; every phase transition is a stop condition plus handoff, not parallel execution.
...
Read `{{runtime_reference_root}}/cards/output-handoff.md` and `{{runtime_reference_root}}/cards/review-verdict.md` before reporting non-trivial workflow status or handing off unresolved work. Report the final review verdict, verification run, skipped checks, blockers, and remaining follow-ups. Do not claim `verdict: READY FOR PR` when the review had no baseline, required verification was skipped, or real-browser/visual/e2e evidence remains relevant but absent.
