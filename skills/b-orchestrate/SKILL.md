...
For non-trivial work, sequencing, risk, public contracts, multi-file edits, or any workflow that needs durable coordination, hand off to `b-plan`. Read `../../b-agentic/references/contract/03-definitions.md` before applying the small-direct threshold. For a small direct workflow, hand off to `b-implement` with the current source of truth, expected scope, and verification need; do not write an execution outline inside `b-orchestrate`.
