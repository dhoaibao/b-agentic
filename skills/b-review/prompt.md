...
Verdicts: **READY FOR PR**, **READY WITH FOLLOW-UPS**, or **NEEDS FIXES**. Emit the chosen label in the final `[status]` block's `verdict:` field after reading `{{runtime_reference_root}}/contract/09-output.md`. Do not use **READY FOR PR** without baseline, required verification, or relevant browser evidence.
