...
2. Inspect the supplied metadata-only path list and targeted non-protected diff evidence. Read repository context only when it materially affects a finding. For a `b-commit` handoff, require the proposed plan with each group's exact paths, message, and any pre-existing staged set; assess group coherence, complete and non-overlapping assignment of intended commit paths (including required generated or prepared files), message accuracy, and applicable commit rules against the frozen candidate. Preserve any pre-existing staged set as one group; reject a plan that splits or reorganizes it. Do not require unrelated worktree changes in the commit plan. An absent or changed plan blocks staging; a candidate-only verdict does not approve the commit plan.
