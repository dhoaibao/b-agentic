...
2. Inspect the supplied metadata-only path list and targeted non-protected diff evidence. Read repository context only when it materially affects a finding. Review does not authorize staging or committing; `b-commit` separately self-audits any commit plan.
