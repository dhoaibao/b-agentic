...
Quick mode stays in chat. For full mode, include durable frontmatter (slug, status, created_at, approved_at, approved_by, approved_head, risk, touch_points) and show the path; ask for approval. Read `{{runtime_reference_root}}/contract/09-output.md` before emitting a status block.
