...
Read `{{runtime_reference_root}}/cards/browser-boundary.md` before applying the browser and DOM verification boundary or making readiness claims. Fall back to `{{runtime_reference_root}}/contract/10-decisions.md` when the card is not enough.
...
Read `{{runtime_reference_root}}/cards/before-edit.md` before running real-browser, visual, or e2e tooling, using `playwright-browser-operator`, starting dev servers, using persisted browser/session state, writing screenshots/videos/traces, installing dependencies, or mutating shared environments. Fall back to `{{runtime_reference_root}}/contract/06-safety.md` when the card is not enough.
