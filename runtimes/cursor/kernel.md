...
# b-agentic - Agent Workflow Kernel for Cursor

> Active always-on runtime rules for routing skills, choosing tools, preserving safety, grounding evidence, verifying work, and handing off cleanly. Detailed schemas, rubrics, MCP bundles, and edge-case protocols live in `~/.cursor/b-agentic/references/contract/` after install and in `references/contract/` in this source repo.

## 0. Runtime Kernel

Use these rules before any skill-specific instruction. If context pressure is high, preserve this kernel first.

Reference checklist: when a kernel rule, skill step, output format, or handoff says to use a schema, rubric, protocol, checklist, or reference section from a b-agentic runtime contract, read the named section file before applying that rule. Adherence is voluntary self-guidance — the runtime has no enforcement hook. Installed skills should read shared contract files from the runtime's shared reference snapshot under `~/.cursor/b-agentic/references/contract/*.md` and skill-local support files from their own directory when a skill points there.

Runtime gate checklist: for non-trivial work, make the gate explicit at the point of use. Before acting, confirm the active skill and source of truth; before editing or external/mutating actions, confirm approval, staleness, worktree, and safety gates; before reporting done or switching skills, confirm verification and read `~/.cursor/b-agentic/references/contract/09-output.md` (§9) when a status block or handoff is required.
...
This runtime contract version is `2026-05-16`. Multi-artifact manifests may include this value as `contract_version`; saved plans use durable approval metadata from §2.
...
Approved saved or chat plans are the execution source of truth for non-trivial implementation. Before executing a saved plan, validate that durable frontmatter is present, `status` is executable or currently approved, `touch_points` match the planned files/areas, the plan is not stale, and every unchecked step has `Done when` verification.
...
Detailed rubrics: runtime contract §3.
...
**Tool fallback:** If required MCP bundles are unavailable, read `~/.cursor/b-agentic/references/contract/04-tool-model.md` before applying fallbacks. Graceful degradation rules and the fallback ladder live there; skills do not restate them.
...
Use the shared slug, run-id, and artifact conventions from the runtime contract §8. Saved plans remain repo-local source-of-truth files; create artifacts only when coordination, evidence, or auditability needs them.

Detailed slug algorithm, paths, manifest schema, and run-id continuity: runtime contract §8.

## 9. Output And Handoffs

Lead with findings, decisions, or the next action. Non-trivial runs use the shared `[status]` and `[handoff]` schemas from `~/.cursor/b-agentic/references/contract/09-output.md`. Save reports only when the user asks, a durable handoff/checkpoint needs one, output is too large for chat, or artifacts require a manifest.

**Status block (shared across all skills):** Read `~/.cursor/b-agentic/references/contract/09-output.md` before emitting any non-trivial status block or handoff; every such run must use the schema defined there. Skills do not restate this rule.
...
Use the shared §10 decision tables for test-vs-bug routing, browser/DOM verification boundaries, snapshot confirmation, flake handling, and cannot-reproduce cases.

Detailed high-risk gate, test-vs-bug table, snapshot procedure, flake handling, browser/DOM verification boundary, and cannot-reproduce protocol: runtime contract §10.
