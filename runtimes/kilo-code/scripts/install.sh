...
readonly METADATA_DIR="${B_AGENTIC_KILO_METADATA_DIR:-$HOME/.kilo/b-agentic}"
readonly BACKUPS_DIR="$METADATA_DIR/backups"
readonly SKILLS_DST="${B_AGENTIC_SKILLS_DST:-$HOME/.kilo/skills}"
...
readonly KILO_JSON_DST="${B_AGENTIC_KILO_JSON:-$HOME/.config/kilo/kilo.jsonc}"
readonly MCP_CONFIG_DST="$KILO_JSON_DST"
...
runtime_upgrade_cli() {
  ...
  if dry_run_enabled; then
    printf '[dry-run] npm install -g @kilocode/cli\n' >&2
  elif command -v npm >/dev/null 2>&1 && npm install -g @kilocode/cli; then
...
manifest = {
    ...
    'paths': {
        ...
        'kiloConfig': os.environ['KILO_JSON_DST'],
...
runtime_print_install_report() {
  ...
  report_item "mcp" "$INSTALL_MCP_ACTION -> $KILO_JSON_DST"
...
runtime_uninstall_configs() {
  ...
  kilo_config_path="$(manifest_path_value kiloConfig "$KILO_JSON_DST")"
