...
run_runtime_smoke_cases() {
  ...
  assert_contains "$sandbox_opencode_install_report/install.log" 'optional-use: readable file previews, YAML-heavy work, better git diffs, and GitHub-heavy workflows'
  ...
  assert_contains "$sandbox_opencode_install_report/install-brew.log" 'core-install: brew install ripgrep fd jq tmux fzf'
  assert_contains "$sandbox_opencode_install_report/install-brew.log" 'optional-install: brew install bat yq git-delta gh'
  ...
  assert_contains "$sandbox_opencode_install_report/install-apt.log" 'core-install: sudo apt install -y ripgrep fd-find jq tmux fzf'
  assert_contains "$sandbox_opencode_install_report/install-apt.log" 'optional-install: sudo apt install -y bat yq git-delta gh'
  ...
  assert_contains "$sandbox_opencode_install_report/install-dnf.log" 'core-install: sudo dnf install -y ripgrep fd-find jq tmux fzf'
  assert_contains "$sandbox_opencode_install_report/install-dnf.log" 'optional-install: sudo dnf install -y bat yq git-delta gh'
  ...
  assert_contains "$sandbox_opencode_install_report/install-manual.log" 'core-install: install manually: ripgrep, fd or fd-find, jq, tmux, fzf'
  assert_contains "$sandbox_opencode_install_report/install-manual.log" 'optional-install: install manually: bat or batcat, yq, git-delta, gh'
