...
Installed detailed refs live under `~/.config/opencode/b-agentic/references/contract/`. They are intentionally few:
...
Read a detailed ref only when the active rule, skill step, output format, or handoff names it or when the kernel is not precise enough. Installed skills read shared refs from `~/.config/opencode/b-agentic/references/contract/` and skill-local support files from their own directory.
...
12. For non-trivial final output or any handoff, read `~/.config/opencode/b-agentic/references/contract/output.md` and use its schema.
...
For overlaps, read `~/.config/opencode/b-agentic/references/contract/runtime.md`.
