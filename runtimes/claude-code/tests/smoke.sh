...
run_runtime_smoke_cases() {
  ...
  local sandbox_bunx_migration="$WORK_DIR/bunx-migration"
  ...
  assert_contains "$sandbox_install_report/install.log" 'optional: bat/batcat, yq, git-delta, gh'
  assert_contains "$sandbox_install_report/install.log" 'optional-use: readable file previews, YAML-heavy work, better git diffs, and GitHub-heavy workflows'
  ...
  HOME="$sandbox_install_report/home" \
  B_AGENTIC_REPO="$snapshot_repo" \
  B_AGENTIC_DIR="$sandbox_install_report/source-brew" \
  B_AGENTIC_PROMPT_API_KEYS=N \
  B_AGENTIC_SHELL_RECOMMEND_MANAGER=brew \
  bash "$ROOT_DIR/install.sh" >"$sandbox_install_report/install-brew.log" 2>&1
  assert_contains "$sandbox_install_report/install-brew.log" 'core-install: brew install ripgrep fd jq tmux fzf'
  assert_contains "$sandbox_install_report/install-brew.log" 'optional-install: brew install bat yq git-delta gh'
  ...
  assert_contains "$sandbox_install_report/install-apt.log" 'core-install: sudo apt install -y ripgrep fd-find jq tmux fzf'
  assert_contains "$sandbox_install_report/install-apt.log" 'optional-install: sudo apt install -y bat yq git-delta gh'
  ...
  assert_contains "$sandbox_install_report/install-dnf.log" 'core-install: sudo dnf install -y ripgrep fd-find jq tmux fzf'
  assert_contains "$sandbox_install_report/install-dnf.log" 'optional-install: sudo dnf install -y bat yq git-delta gh'
  ...
  assert_contains "$sandbox_install_report/install-manual.log" 'core-install: install manually: ripgrep, fd or fd-find, jq, tmux, fzf'
  assert_contains "$sandbox_install_report/install-manual.log" 'optional-install: install manually: bat or batcat, yq, git-delta, gh'
  ...
  assert_json_value "$sandbox_mcp_migration/home/.claude.json" "data['mcpServers']['brave-search']['args'] == ['dlx', '@brave/brave-search-mcp-server', '--transport', 'stdio']"
  assert_json_value "$sandbox_mcp_migration/home/.claude.json" "data['mcpServers']['firecrawl']['command'] == 'pnpm'"
  ...
  assert_json_value "$sandbox_bunx_migration/home/.claude.json" "data['mcpServers']['firecrawl']['command'] == 'pnpm'"
