...
**Tool fallback:** If required MCP bundles are unavailable, read `~/.claude/b-agentic/references/contract/04-tool-model.md` before applying fallbacks. Graceful degradation rules and the fallback ladder live there; skills do not restate them.
...
Lead with findings, decisions, or the next action. Non-trivial runs use the shared `[status]` and `[handoff]` schemas from `~/.claude/b-agentic/references/contract/09-output.md`. Save reports only when the user asks, a durable handoff/checkpoint needs one, output is too large for chat, or artifacts require a manifest.

**Status block (shared across all skills):** Read `~/.claude/b-agentic/references/contract/09-output.md` before emitting any non-trivial status block or handoff; every such run must use the schema defined there. Skills do not restate this rule.
